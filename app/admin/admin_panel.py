...
# ========== FIX 3: Create a reusable auth dependency ==========
...
# ========== FIX 4: Protected routes with proper checks ==========
...
def adminDashboard(request: Request, msg: str = None, db: Session = Depends(get_db)):
    ...
    # ========== FIX 5: Add this check at the beginning of EVERY protected route ==========
    admin_id = request.session.get('admin_id')
    if not admin_id:
        return RedirectResponse(url='/admin/login', status_code=303)
...
def productVault(request: Request, db: Session = Depends(get_db)):
    ...
    # ========== FIX 6: Add session check here too ==========
    admin_id = request.session.get('admin_id')
    if not admin_id:
        return RedirectResponse(url='/admin/login', status_code=303)
...
# ========== FIX 7: Add session check to all POST routes too ==========
...
def completeOrder(order_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(url='/admin_dashboard?msg=Order+Shipped', status_code=303)
...
async def deliverOrder(order_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if order.status == 'delivered':
        return RedirectResponse(url='/admin_dashboard?msg=Order+already+delivered', status_code=303)
    
    # Check stock
    results = db.execute(text("""SELECT product_id, quantity FROM order_items WHERE order_id = :order_id"""), 
                         {'order_id': order_id})
    ...
    return RedirectResponse(url='/admin_dashboard?msg=Order+Delivered', status_code=303)
